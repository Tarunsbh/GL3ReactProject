...
import MovieBatch from '../models/MovieBatch';
...
// console.log(searchText);
   let  searchResultData = [] as MovieBatch[];
   
    
const [ favouriteList, setFavouritelist ] = useState<{ movie: MovieBatch, favourite : number }[]>( [] );
const [ showMovieList, setShowMovieList ] = useState<boolean>(false);
const [ movieDetails, setMovieDetails ] = useState<MovieBatch | null>(null);
const [imageIsOpen , setImageIsOpen] = useState<boolean>(false);

const addToFavourite = (movieValue : MovieBatch) =>
{
    const movie = {
    ...
    } as Omit<MovieBatch, 'id | poster | duration| releaseDate |  originalTitle'>;
...
const removeFromFavourite = (movie : MovieBatch) =>
{
    const  newFavourite = [] as { movie: MovieBatch, favourite : number  }[];
 ...
 if(searchText != null)
 {
    searchResultData = [] as MovieBatch[];
    if(isFavourite)
    {
        searchResultData = [] as MovieBatch[];
    ...
    data[typeOfMovie].forEach(
        p => {
            ...
            if(  p.title.toLowerCase().startsWith(searchText.toLowerCase())) {
                searchResultData.push( p as MovieBatch);
            }
        }
    );
    }
 ...
 else
 {
    ...
    {
        searchResultData = data[typeOfMovie] as MovieBatch[];
    }
 }

 const showMovieDetails = (movie : MovieBatch) =>
 {
